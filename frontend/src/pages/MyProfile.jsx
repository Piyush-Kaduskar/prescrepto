    ...
    <div className='gap-2 text-sm max-w-lg flex flex-col '>
      ...
      <div className=''>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          {
            isEdit ? <p>
              <input className='bg-gray-50' onChange={(e) => setUserData(prev => ({ ...prev, address: { ...prev.address, line1: e.target.value } }))} value={userData.address.line1} type="text" /> <br /> <input className='bg-gray-50' onChange={(e) => setUserData(prev => ({ ...prev, address: { ...prev.address, line2: e.target.value } }))} value={userData.address.line2} type="text" />
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3'>
          ...
          {
            isEdit ? <select className='max-w-20 bg-gray-100 ' onChange={(e) => setUserData(prev => ({ ...prev, gender: e.target.value }))} value={userData.gender} name="" id="">
      ...
      <div className='mt-10'>
         {
        isEdit  ? <button className='hover:bg-primary hover:text-white transition-all duration-400 border border-primary px-8 py-2 rounded-full' onClick={updateUserProfileData}>Save Information</button> : <button className='hover:bg-primary hover:text-white transition-all duration-400 border border-primary px-8 py-2 rounded-full'  onClick={()=>setIsEdit(true)}>Edit</button>
      }
      </div>
    </div>
  )
